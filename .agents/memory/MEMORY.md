...
1. **Above-the-Fold Sightline on Mobile (`390px × 844px`)**:
   ...
   - For homepage (`/`): Eyebrow pill, dynamic headline, subtext, Built Different card/ad, 3 action buttons, 3-column proof rail, and animated marquee ribbon must all fit above the fold.
...
3. **Hero Badge Card on Mobile**:
   - Formatted as a sleek inline "Ad Banner" strip (~42–44px tall) with avatar, gold/blue labels, and Instagram action chip.
4. **Git as Memory**:
